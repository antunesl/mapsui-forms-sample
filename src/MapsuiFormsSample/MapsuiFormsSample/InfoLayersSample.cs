...
namespace MapsuiFormsSample
{
    ...
    public static class InfoLayersSample
    {
        ...
        private static ILayer CreateInfoLayer(BoundingBox envelope, Color color = null)
        {
            return new Layer(InfoLayerName)
            {
                DataSource = RandomPointHelper.CreateProviderWithRandomPoints(envelope, 25, 7),
                Style = CreateSymbolStyle(color)
            };
        }
        ...
        private static SymbolStyle CreateSymbolStyle(Color color = null)
        {
            var symbolColor = color ?? new Color(213, 234, 194);

            return new SymbolStyle
            {
                SymbolScale = 0.8,
                Fill = new Brush(symbolColor),
                Outline = { Color = Color.Gray, Width = 1 }
            };
        }
    }
